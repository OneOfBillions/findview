...
public class InjectWriter extends WriteCommandAction.Simple {
    ...
    public InjectWriter setPackageName(String packageName) {
    ...
    private void generateMultipleClickMethods() {
        for (Element element : mElements) {
            if (element.isClick) {
                StringBuilder method = new StringBuilder();
                method.append("@com.cttrip.bind.COnClick(\"" +packageName+"."+element.getFullID()+ "\")");
    ...
    private void generateSingleClickMethodForSeveralIds() {
        StringBuilder method = new StringBuilder();
        method.append("@com.cttrip.bind.COnClick({");
        int currentCount = 0;
        for (Element element : mElements) {
            if (element.isClick) {
                currentCount++;
                if (currentCount == Utils.getClickCount(mElements)) {
                    method.append("\""+packageName+"."+element.getFullID() + "\"})");
                } else {
                    method.append("\""+packageName+"."+element.getFullID() +  "\",");
                }
            }
        }
        method.append("public void onViewClicked(android.view.View view,final String idName) {switch (idName){");
        for (Element element : mElements) {
            if (element.isClick) {
                method.append("case \"" + packageName+"."+element.getFullID() + "\": break;");
            }
        }
        method.append("}}");
        mClass.add(mFactory.createMethodFromText(method.toString(), mClass));
    ...
    protected void generateAdapter(@NotNull IButterKnife butterKnife) {
        ...
        for (Element element : mElements) {
            ...
            if (element.isAndroidNS) {
                ...
                rPrefix = packageName+".R.id.";
            }

            StringBuilder injection = new StringBuilder();
            injection.append("@CBind");
...
//        mClass.addBefore(mFactory.createCommentFromText(comment.toString(), mClass), mClass.findInnerClassByName(Utils.getViewHolderClassName(), true));
    ...
    protected void generateFields(@NotNull IButterKnife butterKnife) {
        ...
        for (Element element : mElements) {
            ...
            StringBuilder injection = new StringBuilder();
            injection.append("@com.cttrip.bind.CBind");
//            injection.append(butterKnife.getFieldAnnotationCanonicalName());
//            injection.append("(\"");
...
//            method.append(butterKnife.getCanonicalBindStatement());
            method.append("CButterKnife.bind(this);\n");
                ...
                for (PsiStatement statement : onCreate.getBody().getStatements()) {
                    ...
                    if (statement.getFirstChild() instanceof PsiMethodCallExpression) {
                        ...
                        if (methodExpression.getText().equals("setContentView")) {
                            onCreate.getBody().addAfter(mFactory.createStatementFromText("CButterKnife.bind(this);", mClass), statement);
                            break;
                        }
                    }
                }
