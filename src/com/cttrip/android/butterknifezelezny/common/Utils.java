package com.cttrip.android.butterknifezelezny.common;

import com.cttrip.android.butterknifezelezny.Settings;
...
public class Utils {
    ...
    public static   PsiFile resolveFile(Editor editor, PsiFile file,String name) {
        PsiFile layout  = null;
        ...
        PsiElement candidateB = file.findElementAt(offset - 1);
         ...
         project = candidateA.getProject();
        layout = resolveMMLayoutResourceFile(candidateA, project, name);

        if(layout!=null){
            return layout;
        }
         project = candidateB.getProject();
        layout = resolveMMLayoutResourceFile(candidateB, project, name);
        if(layout!=null){
            return layout;
        }
        return layout;
    }
    private static PsiFile resolveMMLayoutResourceFile(PsiElement element, Project project, String name) {
        ...
        if (module != null) {
            ...
            if (files == null || files.length <= 0) {
                ...
                moduleScope = module.getModuleContentScope();
                files = FilenameIndex.getFilesByName(project, name, moduleScope);
            }
        }
        if (files == null || files.length <= 0) {
    ...
    public static  ArrayList<String> getPackageName(Project project,final PsiFile file, ArrayList<String> result) {
            ...
            public void visitElement(final PsiElement element) {
                ...
                if (element instanceof XmlTag) {
                    ...
                    if (tag.getName().equalsIgnoreCase("manifest")) {
                        ...
                        XmlAttribute layout = tag.getAttribute("package", null);
                        if(layout!=null){
                            log.info("found:  package");
                            result.add(layout.getValue());
                            log.info("found value:  "+layout.getValue());


                        }else{

                            log.info("unfound:  package");
